...
export const generateTicketPDF = async (ticketData: TicketData) => {
  ...
  pdf.setFontSize(28);
  const cineMaxWidth = pdf.getTextWidth('CineMax');
  pdf.text('CineMax', (210 - cineMaxWidth) / 2, 20);
  pdf.setFontSize(12);
  const ticketTextWidth = pdf.getTextWidth('Your Movie Ticket');
  pdf.text('Your Movie Ticket', (210 - ticketTextWidth) / 2, 30);
  ...
  pdf.setFontSize(10);
  pdf.text(
    `Date: ${new Date(ticketData.showtime.date).toLocaleDateString('en-US', {
      weekday: 'long',
      year: 'numeric',
      month: 'long',
      day: 'numeric',
